...
def main():
    root = Path.cwd()
    path_results = root.joinpath("results_refined_library")
    models = ["Qwen/Qwen3-8B", "meta-llama/Llama-3.1-8B-Instruct", "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"]
    path_log = path_results.joinpath("log_parsing.txt")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(path_results.joinpath("log_parsing.txt")),
    ...
    evaluate_models(models, path_results, logger)
